# Copied over the synthinfo.rb source code from sonic pi
# in order to swap "module SonicPi" to "class SonicPi" to
# avoid "TypeError Exception: SonicPi is not a module" error

# Files copied:
# https://github.com/samaaron/sonic-pi/blob/master/app/server/ruby/lib/sonicpi/synths/synthinfo.rb
# https://github.com/samaaron/sonic-pi/blob/master/app/server/ruby/lib/sonicpi/util.rb
# https://github.com/samaaron/sonic-pi/blob/master/app/server/ruby/lib/sonicpi/version.rb

# Changes to original source:
# + def self.synth_infos
# +   @@synth_infos
# + end
...
class SonicPi
  module Synths
    class BaseInfo
      include Util
      attr_reader :synth_name, :info

      def initialize
        @cached_arg_info = nil
        @synth_name = "#{prefix}#{synth_name}"
        merged_info = default_arg_info.merge(specific_arg_info)
        @arg_names = arg_defaults.keys

        # useful sanity check
        # @arg_names.each do |k|
        #   raise "no arg info for synth: #{@synth_name}, arg: #{k.inspect}" unless k.to_s.include?("_slide_") || k.to_s == "input" || merged_info[k]
        # end
